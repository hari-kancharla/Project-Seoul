...
public struct DeferredRequest {
    ...
    ///
    /// Travels with the request rather than living in a shared field on the
    /// controller: the whole point of holding a request is that time passes
    /// before it is sent, and the frontmost app can change in that gap. The
    /// answer still belongs to the window that asked.
