...
bool OrganizationModel::ValidIcon(std::string_view icon) const {
  if (icon.empty()) {
    return true;
  }
  if (icon.size() > kMaxIconRefLength || !base::IsStringUTF8(icon) ||
      icon.find("://") != std::string_view::npos ||
      base::StartsWith(icon, "data:", base::CompareCase::INSENSITIVE_ASCII) ||
      base::StartsWith(icon,
                       "javascript:", base::CompareCase::INSENSITIVE_ASCII)) {
    return false;
  }
  return std::ranges::none_of(
      icon, [](unsigned char c) { return c < 0x20 || c == 0x7f; });
}
...
MutationStatus OrganizationModel::SetWorkspaceIcon(const WorkspaceId& id,
  ...
  if (!ValidIcon(icon)) {
    return Err(OrganizationError::kInvalidIcon);
  }
  auto it = workspaces_.find(id);
  if (it == workspaces_.end()) {
    return Err(OrganizationError::kWorkspaceNotFound);
  ...
  it->second.icon = std::string(icon);
  Notify(
      {OrganizationChangeType::kWorkspaceIconChanged, id, TabMembershipId()});
  return Ok();
}
...
MutationStatus OrganizationModel::RebindTabMembership(
    const TabMembershipId& id,
...
MutationStatus OrganizationModel::LoadSnapshot(
  ...
  for (const WorkspaceRecord& w : snapshot.workspaces) {
    if (!w.id.is_valid() || !ValidName(w.name) || !ValidIcon(w.icon)) {
  ...
  for (const EssentialRecord& e : snapshot.essentials) {
    ...
    if (!e.id.is_valid() || !ValidName(e.name) || !ValidIcon(e.icon) ||
        e.root_url.size() > kMaxUrlLength || !parsed_url.is_valid() ||
        !parsed_url.SchemeIsHTTPOrHTTPS()) {
      return Err(OrganizationError::kCorruptState);
    }
    if (!essential_origins.insert(url::Origin::Create(parsed_url).Serialize())
             .second) {
      return Err(OrganizationError::kCorruptState);
    }
