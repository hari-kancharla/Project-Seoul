...
final class SeoulAppController: NSObject {
    ...
    private func fire() {
        ...
        // that was in front when the key was pressed, not when the host turns up.
        ...
        guard server.isConnected else {
            hold(DeferredRequest(requestId: requestId, query: query,
                                 payload: payload, firedAt: t0.uptimeNanoseconds))
            return
        }
        deliver(requestId: requestId, query: query, payload: payload, t0: t0)
    }

    /// Writes one already-encoded request and starts its timing record.
    private func deliver(requestId: String, query: String, payload: Data, t0: DispatchTime) {
    ...
    private func hold(_ request: DeferredRequest) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + seconds + 0.01) { [weak self] in
            ...
            // fire() captured the frontmost app for a sketch that is now never
            // going to be drawn. Left set, the next app switch would report
            // clearing an overlay that was never there — and a log that
            // narrates events which did not happen is worse than no log.
            if !gone.isEmpty && self.deferred.isEmpty && self.liveElementIDs.isEmpty {
                self.annotationOwnerPID = nil
            }
        }
    }
